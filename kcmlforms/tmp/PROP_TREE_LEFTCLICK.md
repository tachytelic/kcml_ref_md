...
This event handler is called if the user left clicks on a node or a leaf within the tree. The [Index](PROP_GENERIC_DATAPTR.htm) property is available within this event handler to determine which item the user clicked on. For example, the following could be used to return the text associated with the item that the user selected: SelItem\$ = ..Item(..Index).Text\$

If there are both LeftClick() and [SelChange()](PROP_TREE_SELCHANGE.htm) event handlers, then only the LeftClick() and not the SelChange() event will be generated when clicking on a tree item.
