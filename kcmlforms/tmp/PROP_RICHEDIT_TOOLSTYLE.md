ToolStyle (richedit control property)
...
If this property is set to ***TRUE*** (the default) then the text formatting buttons are added to the toolbar. If ***FALSE*** then they will not appear and if there are no other toolbar styles enabled ([ToolColor](PROP_RICHEDIT_TOOLCOLOUR.htm) and [ToolFont](PROP_RICHEDIT_TOOLFONT.htm)) then no editing toolbar will be displayed.
