Bind (DataBind control property)
...
This [DataBind](DataBind.htm) control property contains the name of the KCML variable that has been data bound to a [DEFRECORD](mk:@MSITStore:kcmlrefman.chm::/DEFRECORD.htm). Changes made on the form will be propagated automatically to this variable and vice versa. The variable can be either a row buffer for a database table on an instance of a structure described by that record.

A DataBind control is created in the KCML Workbench by dragging the bound record from the record browser and dropping it onto the form's DEFFORM definition in the editor window. This creates a DataBind object with a name of the record name prefixed by 'data\_'. The [.Record](PROP_DATARECORD.htm) property will be set to the name of teh record and the .Bind property will be defaulted to the record name with a \$ sign appended.
