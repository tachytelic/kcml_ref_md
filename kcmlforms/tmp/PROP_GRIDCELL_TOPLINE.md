TopLine (gridcell control property)
...
This property is used to change the line type for the separator line that appears to the top of the current cell or cells specified by the [*Cell()* method. It can also be used to join two cells together vertically. See also the](PROP_GRID_CELLXY.htm) [LeftLine](PROP_GRIDCELL_LEFTLINE.htm) property.
...
For example, the following would change the line to the top of all cells in row 4 to be a thick line: .GridControl1.Cell(4, 0).TopLine = &.ThickLine
